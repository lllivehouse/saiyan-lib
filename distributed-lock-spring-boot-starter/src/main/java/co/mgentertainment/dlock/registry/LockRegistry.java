...
import co.mgentertainment.dlock.utils.CheckedCallable;
import co.mgentertainment.dlock.utils.CheckedRunnable;
    ...
    default <T, E extends Throwable> T executeLocked(Object lockKey, Duration waitLockDuration,
        ...
        if (!lock.tryLock(waitLockDuration.toMillis(), TimeUnit.MILLISECONDS)) {
            throw new TimeoutException(
                    String.format("The lock [%s] was not acquired in time: %s", lockKey, waitLockDuration));
        }
